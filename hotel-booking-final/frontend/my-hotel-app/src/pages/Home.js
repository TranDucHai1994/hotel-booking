 ...
 * Chức năng: Hiển thị giao diện chính, ô tìm kiếm (HeroSearchBar), và danh sách khách sạn.
 * Các logic chính:
 * 1. Gọi API lấy danh sách toàn bộ khách sạn hoặc tìm theo từ khóa.
 * 2. Lọc danh sách ngay trên Frontend dựa theo: Địa điểm, tiện ích (amenities), giá tiền...
 * 3. Sắp xếp danh sách (Giá tăng/giảm, đánh giá).
 */
export default function Home() {
  ...
  useEffect(() => {
    // Hàm gọi API lấy danh sách khách sạn từ backend
  ...
  /**
   * Tính toán lại danh sách khách sạn sẽ hiển thị (Lọc theo điểm đến và tiện ích).
   * Sử dụng useMemo để tối ưu hóa hiệu suất (chỉ tính toán lại khi dữ liệu hotels hoặc bộ lọc thay đổi).
   */
  ...
  // Quay ve trang 1 moi khi bo loc/sap xep/ket qua tim kiem thay doi
  useEffect(() => {
    setPage(1);
  }, [activeDestination, activeFilters, sortValue, hotels]);

  const totalPages = Math.max(1, Math.ceil(sortedHotels.length / PAGE_SIZE));
  const paginatedHotels = useMemo(
    () => sortedHotels.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE),
    [sortedHotels, page]
  );
  ...
  const handlePageChange = (nextPage) => {
    ...
    setPage(clamped);
    document.getElementById('hotel-listing')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };

  /**
   * Hàm xử lý khi người dùng bấm tìm kiếm trên thanh HeroSearchBar.
   * Cập nhật URL (search params) để kích hoạt useEffect lấy dữ liệu mới.
   */
  ...
  return (
    ...
    */}
      {/* ══════════════════════════════════════════
          ...
          ══════════════════════════════════════════ */}
      <section id="hotel-listing" className="bg-background px-4 py-4 sm:py-12 scroll-mt-24">
        <div className="mx-auto max-w-[1380px]">
          {/* Section header */}
            ...
            <>
              ...
              {totalPages > 1 && (
                <nav
                  ...
                  aria-label="Phân trang danh sách khách sạn"
                >
                  <button
                    type="button"
                      ...
                      item === '...' ? (
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            item === page
                              ? 'bg-primary text-white'
                              : 'text-text-secondary hover:bg-hover-surface hover:text-primary'
                          )}
                        >
                          {item}
                        </button>
