...
function buildLargeDemoData() {
  ...
  // Rai booking's created_at qua nhieu nam (co xu huong tang dan + mua cao diem)
  // de bieu do doanh thu theo thang/nam co du lieu truc quan thay vi chi tap trung "hom nay".
  const now = new Date();
  const monthsBack = 42; // ~3.5 nam lich su
  const rangeStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - monthsBack, 1));
  const monthList = [];
  {
    let cursorYear = rangeStart.getUTCFullYear();
    let cursorMonth = rangeStart.getUTCMonth();
    while (cursorYear < now.getUTCFullYear() || (cursorYear === now.getUTCFullYear() && cursorMonth <= now.getUTCMonth())) {
      monthList.push({ year: cursorYear, month: cursorMonth });
      cursorMonth += 1;
      if (cursorMonth > 11) {
        cursorMonth = 0;
        cursorYear += 1;
  ...
  // Diem cong theo mua: dip Tet (thang 1), he (thang 6-7), cuoi nam (thang 12); thang 2 thap diem.
  const seasonalBonus = { 0: 10, 1: -4, 5: 6, 6: 7, 11: 12 };

  let bookingCounter = 0;

  monthList.forEach((monthInfo, monthIndex) => {
    const { year, month } = monthInfo;
    const daysInMonth = new Date(Date.UTC(year, month + 1, 0)).getUTCDate();
    const isCurrentMonth = year === now.getUTCFullYear() && month === now.getUTCMonth();
    const maxDay = isCurrentMonth ? now.getUTCDate() : daysInMonth;

    const growth = monthIndex * 0.4;
    const bonus = seasonalBonus[month] || 0;
    let monthlyCount = Math.round(16 + growth + bonus);
    if (isCurrentMonth) {
      monthlyCount = Math.round(monthlyCount * (maxDay / daysInMonth));
    }
    monthlyCount = Math.max(6, monthlyCount);

    for (let i = 0; i < monthlyCount; i += 1) {
      const customer = generatedCustomers[bookingCounter % generatedCustomers.length];
      const room = generatedRooms[bookingCounter % generatedRooms.length];
      const day = 1 + ((bookingCounter + i * 7) % maxDay);
      const hour = (bookingCounter * 3 + i) % 24;
      const minute = (i * 11) % 60;
      const createdAt = new Date(Date.UTC(year, month, day, hour, minute));

      const leadDays = 2 + (bookingCounter % 30);
      const stayStart = new Date(createdAt);
      stayStart.setUTCDate(stayStart.getUTCDate() + leadDays);
      const nights = 1 + (bookingCounter % 4);
      const stayEnd = new Date(stayStart);
      stayEnd.setUTCDate(stayEnd.getUTCDate() + nights);

      const paymentMethod = ['mock_card', 'mock_momo', 'pay_at_hotel'][bookingCounter % 3];
      const isFutureStay = stayStart > now;
      const status = bookingCounter % 9 === 0
        ? 'cancelled'
        : isFutureStay && bookingCounter % 3 === 0
          ? 'pending'
          : 'confirmed';
      const paymentStatus = status === 'cancelled' && paymentMethod !== 'pay_at_hotel'
        ? 'refunded'
        : paymentMethod === 'pay_at_hotel'
          ? (isFutureStay ? 'unpaid' : 'paid')
          : 'paid';
      ...
      bookingsSeed.push({
        ...
        guests: 1 + (bookingCounter % Math.max(1, room.max_guests)),
        status,
        payment_method: paymentMethod,
        payment_status: paymentStatus,
        customer_note: `Demo booking #${bookingCounter + 1} for ${room.room_type}.`,
